...
def plotNetworkTompkins(nodes, links):
    ...
    graph.node_renderer.glyph = Circle(line_color='steelblue', line_width=2, fill_color='lightblue', radius=10)
    graph.node_renderer.hover_glyph = Circle(line_color='lightgreen', line_width=2, fill_color='yellow', radius=10)
    graph.node_renderer.selection_glyph = Circle(line_color='orange', line_width=2, fill_color='yellow', radius=10)
    # define edges
...
def plotTreeTompkins(dfn, dfl):
    ...
    # style nodes
    graph.node_renderer.glyph = Circle(line_color='green', line_width=2, fill_color='lightgreen', radius=10)
    graph.node_renderer.hover_glyph = Circle(line_color='lightgreen', line_width=2, fill_color='yellow', radius=10)
    graph.node_renderer.selection_glyph = Circle(line_color='orange', line_width=2, fill_color='yellow', radius=10)
    # style edges
...
def plotNetwork(nodes, links, title='Plot of Graph', targets=None, on_map=False):
    ...
    poi = Circle(x="x", y="y", radius=250, line_color='black',
...
def plotShortestPathTree(dfn, dfl, out, targets=None):
    ...
    if targets != None:
        ...
        dfp = dfn.loc[dfn['name'].isin(targets)]
        source = ColumnDataSource(dfp)
        pois = Circle(x="x", y="y", radius=7, line_color='orange',
