...
class TaxiRouting(object):
    ...
    def load_model(self):
        ...
        self.model = min_cost_flow.SimpleMinCostFlow()
        ...
        for i in range(len(tails)):
            self.model.add_arc_with_capacity_and_unit_cost(tails[i], heads[i], caps[i], costs[i])
        ...
        for i in range(0, len(supplies)):
            self.model.set_node_supply(i, supplies[i])
    ...
    def optimize(self):   
        ...
        self.objective = -self.model.optimal_cost()/100
...
#         return taxi_time_dict
    ...
    def plot_taxi_route(self, taxis):
        ...
        graph.node_renderer.glyph = Circle(radius=14,line_width=0,fill_alpha=1, fill_color='start_colors')
    ...
    def plot_heatmap(self):
        ...
        plot = figure(x_range=(min_x, max_x), y_range=(min_y, max_y),
                      x_axis_type="mercator", y_axis_type="mercator",
                      title='Heatmap of Taxi Pickups / Dropoffs', width=600, height=470)
        plot.add_tile("CartoDB Positron retina")
        ...
        graph.node_renderer.glyph = Circle(radius=200,line_width=0,fill_alpha=0.5/self.T_max, fill_color='colors')
        # graph.node_renderer.glyph = Circle(radius=7,line_width=0,fill_alpha=0.5, fill_color='colors')
...
#         trips_df = trips_df.append(pd.DataFrame(row, index=[0]))
        trips_df = pd.concat([trips_df, pd.DataFrame(row, index=[0])], ignore_index=True)
...
#         nodes_df = nodes_df.append(pd.DataFrame(row, index=[0]))
        nodes_df = pd.concat([nodes_df, pd.DataFrame(row, index=[0])], ignore_index=True)
...
#         arcs_df = arcs_df.append(pd.DataFrame(row, index=[0]))
        arcs_df = pd.concat([arcs_df, pd.DataFrame(row, index=[0])], ignore_index=True)
